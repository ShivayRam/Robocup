...
class Agent(Base_Agent):
    ...
    def beam(self, avoid_center_circle=False):
        r = self.world.robot
        pos = self.init_pos.copy() # copy position list 
        self.state = 0
...
#functions to handle our and their goal kicks
    ...
    def select_skill(self,strategyData):
        ...
        #checks if formation cache is still valid and game mode hasnt changed
        curr_time = self.world.time_local_ms
        formation_cache_valid = (curr_time - self.last_decision_time < 200 and 
        ...
        if not formation_cache_valid:
            ...
            self.last_decision_time = curr_time
            self.last_play_mode = strategyData.play_mode
        ...
        if strategyData.active_player_unum == strategyData.robot_model.unum: # I am the active player 
            ...
            URGENT_SHOOT_DISTANCE_SQ = 64
            SHOOT_DISTANCE_SQ = 144
            POSITIONING_DISTANCE_SQ = 196
            ...
            if dist_to_goal_sq < URGENT_SHOOT_DISTANCE_SQ:
                ...
                if abs(mypos[1]) < 2:
                    ...
                    shoot_target = np.array([15, -1.2 if mypos[1] > 0 else 1.2])
                ...
                if abs(mypos[1]) < 3:
                    shoot_target = np.array([15, -0.5 if mypos[1] > 0 else 0.5])
                
                else:
                    shoot_target = np.array([15, -1 if mypos[1] > 0 else 1])
                ...
                if mypos[1] > 0:
                    pos_target = np.array([min(mypos[0] + 2, 13), -1])
                ...
                advance_vec = advance_vec / np.sqrt(dist_to_goal_sq) * 3
